...
export async function GET(req:NextRequest){
 ...
 const [listingsR,invitesR,eventsR]=await Promise.all([
   fetch(`${URL}/rest/v1/cm_listings?select=id,slug,title,category,neighborhood,starts_at,source_type,claimed,status,expires_at&status=eq.active&expires_at=gt.${encodeURIComponent(new Date().toISOString())}&order=starts_at.asc`,{headers:ctx.headers,cache:'no-store'}),
   fetch(`${URL}/rest/v1/cm_alpha_invites?select=code,label,bonus_free_posts,max_uses,uses,active,created_at,expires_at&order=created_at.desc&limit=30`,{headers:ctx.headers,cache:'no-store'}),
   fetch(`${URL}/rest/v1/cm_product_events?select=event_name,occurred_at&occurred_at=gt.${encodeURIComponent(new Date(Date.now()-7*86400000).toISOString())}`,{headers:ctx.headers,cache:'no-store'})
 ]);
 const listings=listingsR.ok?await listingsR.json():[];const invites=invitesR.ok?await invitesR.json():[];const events=eventsR.ok?await eventsR.json():[];
 const targets:Record<string,number>={'HOY':12,'ESTA NOCHE':8,'MAÑANA':8,'FINDE':15,'PRÓXIMOS':20};const windows=Object.keys(targets).map(k=>{const count=listings.filter((x:any)=>bucket(x.starts_at)===k).length;return{key:k,count,target:targets[k],status:count>=targets[k]?'green':count>=Math.ceil(targets[k]*.55)?'yellow':'red'}});
 const neighborhoods=Object.entries(listings.reduce<Record<string,number>>((a:any,x:any)=>{a[x.neighborhood]=(a[x.neighborhood]||0)+1;return a},{})).sort((a,b)=>b[1]-a[1]).slice(0,8);
 const categories=Object.entries(listings.reduce<Record<string,number>>((a:any,x:any)=>{a[x.category]=(a[x.category]||0)+1;return a},{})).sort((a,b)=>b[1]-a[1]);
 const funnel=events.reduce<Record<string,number>>((a:any,x:any)=>{a[x.event_name]=(a[x.event_name]||0)+1;return a},{});
 return NextResponse.json({data:{windows,neighborhoods,categories,funnel,invites,communitySeed:listings.filter((x:any)=>x.source_type==='community').length,total:listings.length}},{headers:{'Cache-Control':'no-store'}});
}
...
export async function POST(req:NextRequest){
 ...
 if(body.action==='create_invite'){
   const code=('CM'+Math.random().toString(36).slice(2,8)).toUpperCase();const row={code,label:(body.label||'Alpha organizer').slice(0,80),bonus_free_posts:Math.min(20,Math.max(1,Number(body.bonus||5))),max_uses:Math.min(100,Math.max(1,Number(body.maxUses||1))),active:true};
   const r=await fetch(`${URL}/rest/v1/cm_alpha_invites`,{method:'POST',headers:{...ctx.headers,Prefer:'return=representation'},body:JSON.stringify(row),cache:'no-store'});if(!r.ok)return NextResponse.json({error:'invite_create_failed',detail:(await r.text()).slice(0,200)},{status:400});const data=await r.json();return NextResponse.json({data:data[0]});
 }
