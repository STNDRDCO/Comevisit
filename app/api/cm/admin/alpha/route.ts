...
export async function GET(req:NextRequest){
 ...
 const listings:AlphaListing[]=listingsR.ok?await listingsR.json():[];const invites=invitesR.ok?await invitesR.json():[];const events:ProductEvent[]=eventsR.ok?await eventsR.json():[];
 ...
 return NextResponse.json({data:{windows,neighborhoods,categories,funnel,invites,communitySeed:listings.filter(x=>x.source_type==='community').length,total:listings.length}},{headers:{'Cache-Control':'no-store'}});
}
