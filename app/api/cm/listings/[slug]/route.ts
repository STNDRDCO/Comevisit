...
function destinationType(raw?:string){
  return (raw||'web').toLowerCase().replace('checkout / entradas','checkout');
}
...
export async function PATCH(req:NextRequest,{params}:{params:Promise<{slug:string}>}){
  ...
  if(body.action==='cancel'){
    const patch=await fetch(`${URL}/rest/v1/cm_listings?slug=eq.${encodeURIComponent(slug)}&owner_id=eq.${user.id}`,{method:'PATCH',headers:{...base,Authorization:`Bearer ${token}`,Prefer:'return=representation'},body:JSON.stringify({status:'cancelled',updated_at:new Date().toISOString()}),cache:'no-store'});
    if(!patch.ok)return NextResponse.json({error:'cancel_failed'},{status:patch.status});
    const rows=await patch.json();
    if(!rows.length)return NextResponse.json({error:'not_found_or_forbidden'},{status:404});
  ...
  const destType=destinationType(body.destinationType);
  if(!allowedDestinations.has(destType))return NextResponse.json({error:'invalid_destination'},{status:400});
  let destUrl:string;
  try{destUrl=new URL(body.destination).toString()}catch{return NextResponse.json({error:'invalid_url'},{status:400})}
  ...
  const patch=await fetch(`${URL}/rest/v1/cm_listings?slug=eq.${encodeURIComponent(slug)}&owner_id=eq.${user.id}`,{method:'PATCH',headers:{...base,Authorization:`Bearer ${token}`,Prefer:'return=representation'},body:JSON.stringify(update),cache:'no-store'});
