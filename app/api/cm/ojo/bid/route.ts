...
export async function POST(req:NextRequest){
  ...
  const lookup=await fetch(`${URL}/rest/v1/cm_listings?select=id,slug,title&slug=eq.${encodeURIComponent(body.listing)}&limit=1`,{headers,cache:'no-store'});if(lookup.status===401)return NextResponse.json({error:'invalid_session'},{status:401});const rows=lookup.ok?await lookup.json():[];if(!rows[0])return NextResponse.json({error:'listing_not_owned_or_inactive'},{status:404});
  const rpc=await fetch(`${URL}/rest/v1/rpc/cm_place_ojo_bid`,{method:'POST',headers,body:JSON.stringify({p_listing_id:rows[0].id,p_market_key:body.market,p_target_amount:target}),cache:'no-store'});
