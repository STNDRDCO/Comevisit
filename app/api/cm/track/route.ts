...
const allowed=new Set(['home_view','explore_view','publish_start','publish_blocked','save','unsave','share','seen','ojo_view','ojo_bid_attempt','alpha_invite_view','alpha_invite_redeemed','presence']);
...
export async function POST(req:NextRequest){
  ...
  if(body.event==='seen'&&!body.sessionId)return NextResponse.json({error:'session_required'},{status:400});
  ...
  if(body.event==='seen'&&!listingId)return NextResponse.json({error:'listing_required'},{status:400});
  ...
  if(!write.ok){
    if(body.event==='seen'&&write.status===409)return NextResponse.json({ok:true,duplicate:true});
    return NextResponse.json({error:'track_failed'},{status:502});
  }
