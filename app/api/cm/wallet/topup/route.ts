...
export async function POST(req:NextRequest){
  ...
  const authHeaders={...base,Authorization:`Bearer ${token}`};
  const create=await fetch(`${SUPABASE_URL}/rest/v1/rpc/cm_create_topup`,{method:'POST',headers:authHeaders,body:JSON.stringify({p_amount_minor:amount}),cache:'no-store'});
  ...
  const preference={
    items:[{id:'cm_ojo_balance',title:'Saldo CHE, MIRÁ · Ojo Acá',description:'Saldo para competir por atención patrocinada en Ojo Acá',quantity:1,currency_id:'ARS',unit_price:amount}],
    payer:user.email?{email:user.email}:undefined,
    external_reference:topup.id,
    metadata:{cm_topup_id:topup.id},
    back_urls:{success:`${appUrl}/che-mira-v5/wallet?payment=success`,pending:`${appUrl}/che-mira-v5/wallet?payment=pending`,failure:`${appUrl}/che-mira-v5/wallet?payment=failure`},
    auto_return:'approved',
    notification_url:`${appUrl}/api/cm/payments/mercadopago/webhook`
  };
  ...
  if(!mpRes.ok)return NextResponse.json({error:'mercadopago_preference_failed',detail:mpData},{status:502});
  ...
  await fetch(`${SUPABASE_URL}/rest/v1/rpc/cm_attach_topup_preference`,{method:'POST',headers:authHeaders,body:JSON.stringify({p_topup_id:topup.id,p_preference_id:mpData.id,p_init_point:checkout}),cache:'no-store'}).catch(()=>{});
