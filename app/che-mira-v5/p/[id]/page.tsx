...
type RemoteListing={slug:string;title:string;category:string;neighborhood:string;venue:string|null;starts_at:string;price_label:string|null;description:string|null;destination_type:string;destination_url:string;claimed:boolean;source_type:string};

const legacy:Record<string,Detail>={
...
const alias:Record<string,string>={'cata-vinos-naturales':'cata','dj-set-vinilos':'dj','menu-parrilla-pasos':'parrilla','standup-en-vivo':'standup','taller-pasta-fresca':'pasta','feria-diseno-independiente':'feria','jazz-en-un-living':'jazz','brunch-de-autor':'brunch','festival-japones':'festival','sunset-rooftop':'rooftop'};
const cta=(type:string)=>type==='instagram'?'Ver Instagram ↗':type==='whatsapp'?'Reservar por WhatsApp ↗':type==='checkout'?'Comprar entrada ↗':'Ir a la web ↗';
...
const fromRemote=(x:RemoteListing):Detail=>({id:x.slug,title:x.title,date:formatDate(x.starts_at),place:x.neighborhood,venue:x.venue||'Ubicación informada por quien publicó',category:x.category,price:x.price_label||'Consultar',detail:x.description||'',cta:cta(x.destination_type),destination:x.destination_url,claimed:x.claimed,organizer:x.claimed?'Publicación reclamada':x.source_type==='community'?'Subido por la comunidad':'Subido por el organizador'});

export default function ListingDetail(){
  const params=useParams<{id:string}>();
  const fallback=legacy[params.id]||legacy[alias[params.id]]||null;
  const [item,setItem]=useState<Detail|null>(fallback);
  const [loading,setLoading]=useState(true);
  const [saved,setSaved]=useState(false);
  const [copied,setCopied]=useState(false);

  useEffect(()=>{
    const requested=alias[params.id]?params.id:Object.entries(alias).find(([,v])=>v===params.id)?.[0]||params.id;
    fetch(`/api/cm/listings?slug=${encodeURIComponent(requested)}`).then(r=>r.ok?r.json():Promise.reject()).then(({data})=>{if(data)setItem(fromRemote(data))}).catch(()=>{}).finally(()=>setLoading(false));
  ...
  const share=async()=>{if(!item)return;const url=window.location.href;if(navigator.share){await navigator.share({title:item.title,url});return;}await navigator.clipboard?.writeText(url);setCopied(true);setTimeout(()=>setCopied(false),1400)};

  if(loading&&!item)return <main className="detail"><header className="detailTop"><Link href="/che-mira-v5" className="detailLogo">CHE, MIRÁ</Link></header><section className="notFound"><h1>Cargando…</h1></section></main>;
  ...
  return <main className="detail">
    ...
    <section className="detailHero"><div className="detailMain"><div className="detailMeta"><span>{item.date}</span><b>{item.category}</b></div><h1>{item.title}</h1><p className="lead">{item.detail}</p><div className="facts"><div><small>DÓNDE</small><strong>{item.place}</strong><span>{item.venue}</span></div><div><small>PRECIO</small><strong>{item.price}</strong></div></div><a className="primaryCta" href={item.destination} target="_blank" rel="noreferrer">{item.cta}</a><div className="utility"><button onClick={toggleSave}>{saved?'♥ Guardado':'♡ Guardar'}</button><button onClick={share}>{copied?'Link copiado ✓':'Compartir ↗'}</button><button>Agregar al calendario</button></div></div>
      <aside className="detailSide"><span>PUBLICACIÓN</span><div className={item.claimed?'claim claimed':'claim'}><b>{item.claimed?'✓ RECLAMADA':'SIN RECLAMAR'}</b><p>{item.organizer}</p></div>{!item.claimed&&<Link href={`/che-mira-v5/reclamar?listing=${encodeURIComponent(item.id)}`} className="claimBtn">¿Esto es tuyo? Reclamar</Link>}<div className="neutral"><b>Che, Mirá no recomienda.</b><p>Esta ficha existe para ordenar la información y llevarte al destino que eligió quien publicó.</p></div><button className="report">Reportar publicación</button></aside></section>
    <section className="more"><div><span>YA QUE ESTÁS ACÁ</span><h2>Che, mirá qué más hay.</h2></div><div className="moreLinks"><Link href="/che-mira-v5#ojo">Ojo Acá →</Link><Link href="/che-mira-v5#recien">Recién publicado →</Link><Link href="/che-mira-v5#explorar">Explorar todo →</Link></div></section>
  </main>
}
