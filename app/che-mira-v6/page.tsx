...
export default function CheMiraV6(){
 ...
 <section className="cm6AttentionHero"><span>HOY · BUENOS AIRES</span><h1>Hoy hay <em>{pulse?money(attentionToday):'—'}</em> tratando de llamar tu atención.</h1><p>{paidToday?`${plural(paidToday,'publicación pagó','publicaciones pagaron')} para aparecer primero.`:'Todavía nadie pagó para aparecer primero.'} A las 6:00, arranca otro día.</p></section>
 ...
 <section id="ranking" className="cm6Market"><div className="cm6MarketHead"><span>PAGARON PARA QUE MIRES PRIMERO</span><div><b>{plural(paid.length,'publicación compitiendo','publicaciones compitiendo')}</b><strong>{money(attentionToday)}</strong></div></div>
