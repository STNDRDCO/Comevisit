...
type ShareContext={rank?:number;amount?:number};
...
export default function CheMiraV6(){
 ...
 const share=async(item:Listing,context?:ShareContext)=>{const url=`${location.origin}/che-mira-v6/p/${item.slug}?src=share`;const text=context?.rank&&context.amount!=null?`#${context.rank} hoy en CHE, MIRÁ · ${attentionValue(context.amount,currency,rates)} puestos para llamar tu atención. ${item.title}`:`Hoy en CHE, MIRÁ · ${item.title}`;let completed=false;try{if(navigator.share){await navigator.share({title:item.title,text,url});completed=true}else{await navigator.clipboard.writeText(`${text}\n${url}`);completed=true;setShared(item.slug);setTimeout(()=>setShared(''),1800)}}catch{}if(completed){setStats(s=>({...s,[item.slug]:{...(s[item.slug]||zeroStat),shares:(s[item.slug]?.shares||0)+1}}));fetch('/api/cm/track',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({event:'share',sessionId:sessionId(),listing:item.slug,properties:{surface:'v6.3_home',rank:context?.rank||null,amount:context?.amount||null}})}).catch(()=>{})}};
 const revealFree=showFree||paid.length===0;
 const social=(item:Listing,context?:ShareContext)=>{const st=stats[item.slug]||zeroStat;const didSee=seen.includes(item.slug);return <div className="cm6Social"><span className="cm6Views" title="Aperturas de esta publicación">◉ {st.views}</span><button className={didSee?'seen':''} onClick={()=>markSeen(item.slug)}>{didSee?'LO VI ✓':'LO VI'}{st.seen>0?` · ${st.seen}`:''}</button><button onClick={()=>share(item,context)} title="Compartir">{shared===item.slug?'COPIADO ✓':`↗${st.shares>0?` ${st.shares}`:''}`}</button></div>};
 ...
 {!loading&&!error&&paid.length>0&&<div className="cm6PaidList">{paid.map((x,i)=>{const item=x.listing!,sig=signalFor(item),platform=platformFor(item);return <article className="cm6PaidRow" key={item.slug}><div className="cm6Rank">#{i+1}</div><div className="cm6Kind hot" title={sig.label}><SignalIcon kind={sig.kind}/></div><div className="cm6PaidCopy"><div className="cm6EventMeta"><span>{eventLabel(item.starts_at)}</span><b>{sig.label}</b><a className="cm6Source" href={`/api/cm/go?slug=${encodeURIComponent(item.slug)}`} target="_blank" rel="noreferrer"><PlatformIcon kind={platform.kind}/>{platform.label}</a></div><h2><Link href={`/che-mira-v6/p/${item.slug}?src=ranking`}>{item.title}</Link></h2><p>{item.neighborhood}{item.description?` · ${item.description}`:''}</p>{social(item,{rank:i+1,amount:x.amount_minor})}</div><div className="cm6Bid"><small>PUSO</small><strong>{attentionValue(x.amount_minor,currency,rates)}</strong></div></article>})}</div>}
