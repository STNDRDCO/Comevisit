...
const organic=[
  {name:'El Preferido',area:'Palermo',kind:'Bodegón',note:'Clásicos porteños, vermut, milanesa y una sala que tiene personalidad propia.'},
  {name:'Atte. Pizzeria',area:'Chacarita',kind:'Pizza',note:'Pizza porteña contemporánea. Crujiente, filosa y sin disfrazarse de Nápoles.'},
  {name:'Naranjo Bar',area:'Chacarita',kind:'Vino',note:'Vino natural, platos chicos y energía de barrio.'},
  {name:'Julia',area:'Villa Crespo',kind:'Autor',note:'Pocas mesas, cocina seria y esa sensación de que te mandó alguien que sabe.'},
  {name:'Don Julio',area:'Palermo',kind:'Parrilla',note:'El peso pesado. No necesita ComeVisit. Por eso todos quieren sacarle la corona.'},
  {name:'La Mezzetta',area:'Villa Ortúzar',kind:'Pizza',note:'Fugazzeta, mostrador y Buenos Aires sin filtro.'},
  {name:'El Ferroviario',area:'Liniers',kind:'Parrilla',note:'Parrilla enorme, mesas largas y otra Buenos Aires muy lejos del circuito obvio.'},
  {name:'Café Paulín',area:'Centro',kind:'Bodegón',note:'Barra, sánguches y centro porteño en estado puro.'},
];
...
export default function V5(){
  return <main className="v5">
    ...
    <section className="marketNav">{markets.map((m,i)=><button className={i===0?'active':''} key={m[0]}><span>{m[0]}</span><b>{m[1]}</b></button>)}</section>

    <section className="crown">
      <div className="crownTitle"><span>♛ CORONA ARGENTINA · PATROCINADO · PUJA PÚBLICA</span><h2>#1 Don Julio</h2><p>Palermo · Parrilla · ocho movimientos públicos esta semana</p></div>
      <div className="crownPrice"><small>PRECIO DE LA CORONA</small><b>US$188</b><button>QUEDATE CON EL #1 POR US$189 ↗</button></div>
    </section>
    ...
    <section className="organic">
      <div className="sectionHead"><span>EL MAPA QUE HAY ABAJO</span><h2>Lugares que valen la pena, paguen o no.</h2><p>Estos son ejemplos temporales mientras resolvemos la importación exacta de la lista compartida de Google Maps. En el producto real, aparecer y pagar son dos cosas distintas.</p></div>
    ...
    <footer><Link href="/lab">← LAB DE PRODUCTO</Link><span>V5.1 ARGENTINA · PUJAS DEMO · SIN PAGOS REALES</span></footer>
  </main>
}
